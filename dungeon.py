...
class Generator():
  def __init__(self, width=64, height=64, max_rooms=15, min_room_xy=5,
    ...
    self.tiles_num = CHARACTER_TILES_NUM
    self.level = []
    self.room_list = []
    self.corridor_list = []
    self.tiles_level = []
  ...
  def print_dungen(self):
    # print('Room List: ', self.room_list)
    # print('\nCorridor List: ', self.corridor_list)

    [print(row) for row in self.tiles_level]
 
  def get_dungen_array(self):
    dungeon = []
    for row_num, row in enumerate(self.level):
      dungeon.append([])
      for col_num, col in enumerate(row):
        if col == 'stone':
          dungeon[row_num].append(self.tiles_num['stone'])
        if col == 'floor':
          dungeon[row_num].append(self.tiles_num['floor'])
        if col == 'wall':
          dungeon[row_num].append(self.tiles_num['wall'])
    return dungeon
